...
public class NBodyGas {
	...
	public static void main(String[] args) throws IOException {	
		...
		//data for RDF, an array with 0.01 distance increments seems reasonable
		...
		for (int i = 0; i < stepNo; i++) {
			...
			//only print at specific frequency, only print during the LAST 10000 steps
			if ((i+1) % outputFrequency == 0 && (stepNo-i<=10000)) {
		...
		for (int i = 0; i < deltaPeaks.length; i++) {
			//normalize RDF, only final 10000 steps are recorded
			deltaPeaks[i] = deltaPeaks[i] * outputFrequency / 10000;			
		}
		...
		System.out.println(stepNo + " step simulation ran in: " + (System.currentTimeMillis()-startTime)/1000 + " seconds");
	}
}
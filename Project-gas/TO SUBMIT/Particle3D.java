...
public class Particle3D {
	...
	public void jumpVelocity(double dt, Vector3D force) {		
		Vector3D forceCopy = new Vector3D(force);
		forceCopy.scalarMultiply(dt/mass);
		velocity = Vector3D.vectorAdd(velocity, forceCopy);		
	}
	...
	public static Vector3D[] forceArrayLJ(Particle3D[] particles, Vector3D boxSize, double cutoffDistance) {
		...
		for (int i = 0; i < forces.length; i++) {
			...
			forces[i] = emptyVector;
		}
		
		for (int i = 0; i < particles.length-1; i++) {
			for (int j = i + 1; j < particles.length; j++) {
				Vector3D force12 = Particle3D.LJForce(particles[i], particles[j], boxSize, cutoffDistance);
				forces[i] = Vector3D.vectorAdd(forces[i], force12);
				//Same, but opposite direction contribution on the j-th particle (Newton 3)
				forces[j] = Vector3D.vectorSubtract(forces[j], force12);
			}
		}			
		return forces;		
	}
	...
	public static void jumpPositionArrayVerlet(double dt, Particle3D[] particles, Vector3D[] forces, Vector3D boxSize) {
		...
		for (int i = 0; i < particles.length; i++) {	
			...
			//modolo operator here can return negative values
			...
			Vector3D new_position = new Vector3D(newX, newY, newZ);						
	...
	public static double[] energyArray(Particle3D[] particles, Vector3D boxSize, double cutoffDistance) {
		...
		//*4 correction as this multiplication is skipped at potential calculation
		energies[1] = 4*totPotential;		
		return energies;
	}
	...
	public static Vector3D relativeSeparation(Particle3D p1, Particle3D p2) {
		Vector3D separation = new Vector3D(Vector3D.vectorSubtract(p1.getPosition(), p2.getPosition()));
		return separation;
	}
	 ...
	 * @param p2 Particle3D, the second particle
	 * @param boxSize Vector3D, a vector holding the dimensions of the simulation box
	 * @return the relative separation vector r = r1 - r2 taking MIC into account
	 */
	public static Vector3D relativeSeparationMIC(Particle3D p1, Particle3D p2, Vector3D boxSize) {
		
		Vector3D separation = new Vector3D(relativeSeparation(p1, p2));			
	...
	public static Vector3D LJForce(Particle3D p1, Particle3D p2, Vector3D boxSize, double cutoffDistance){
		Vector3D force = Particle3D.relativeSeparationMIC(p1, p2, boxSize);
		double r2 = force.magnitudeSquared();		
		
		//Basic, uncorrected LJ force
		
		/*
		double magnitude = 48*(Math.pow(r2, -7) - Math.pow(r2, -4)/2);
		force.scalarMultiply(magnitude);	
		*/
		
		//truncated potential, force unshifted		
		...
		//truncated potential, force shifted to be 0 at cutoffDistance
		
		/*
		double r = Math.sqrt(r2);
		force.scalarDivide(r);	
		if (r < cutoffDistance) {	
			double forceCorr = Math.pow(cutoffDistance, -13) - Math.pow(cutoffDistance, -7)/2;
			double magnitude = 48*(Math.pow(r, -13) - Math.pow(r, -7)/2 - forceCorr);
			force.scalarMultiply(magnitude);				
		} else {
			force = new Vector3D(0,0,0);
		}
	...
	public static double LJPotential(Particle3D p1, Particle3D p2, Vector3D boxSize, double cutoffDistance) {
		double r2 = Particle3D.relativeSeparationMIC(p1, p2, boxSize).magnitudeSquared();
		double potential = 0;			
		
		//avoiding square roots if possible
		//using the factor 4 only when the total PE is calculated
		
		//Truncated potential		
		
		if (r2 < cutoffDistance*cutoffDistance) {
			double discCorr = Math.pow(cutoffDistance, -12) - Math.pow(cutoffDistance, -6);
			potential = Math.pow(r2, -6) - Math.pow(r2, -3) - discCorr;
		}	
		
		return potential;	
		
		//Simple potential without corrections
		
		/*
		potential = Math.pow(r2, -6) - Math.pow(r2, -3);
		*/
		
		//---------------------------------------------------------
		
		//Truncated, force shifted potential
		
		/*
		double r = Math.sqrt(r2);
		if (r < cutoffDistance) {
			double discCorr = Math.pow(cutoffDistance, -12) - Math.pow(cutoffDistance, -6);
			double forceCorr = 12*(Math.pow(cutoffDistance, -13) - Math.pow(cutoffDistance, -7)/2);
			potential = Math.pow(r2, -6) - Math.pow(r2, -3) - discCorr + forceCorr*(r-cutoffDistance);
		} 
		*/		
	}
	 ...
	 * distance to the length of the RDF array. As the rdf describes density variation with
	 * r, there is a 1/r^2 factor 
	 * 
	...
	public static void calculateRDF(Particle3D[] particles, Vector3D boxSize, double[] deltaPeaks, double cutoffDistance) {
		...
		double sepToIndex = 100;
		
		//i=0 to N-1, j=i+1 to N looping ensures the j not equal to i condition is satisfied
		for (int i = 0; i < particles.length-1; i++) {						
			for (int j = i+1; j < particles.length; j++) {	
				double thisSep = Particle3D.relativeSeparationMIC(particles[i], particles[j], boxSize).magnitude();
				double deltaPeak = thisSep * sepToIndex;
				...
				if (arrayPos < deltaPeaks.length) {
					...
					deltaPeaks[arrayPos] = deltaPeaks[arrayPos] + 1D/(thisSep*thisSep);
				}				
			}			
		}		
	}

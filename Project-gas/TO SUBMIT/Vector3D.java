...
public class Vector3D {
	 ...
	 * @param v1 Vector3D, a vector
	 * @param v2 Vector3D, a vector
	 * @return true if they are (up to a small margin) equal, false otherwise
	 */
	public static boolean equal(Vector3D v1, Vector3D v2) {
		double margin = 1E-7;
		if (Math.abs(v1.getX()-v2.getX())<margin && Math.abs(v1.getY()-v2.getY())<margin && Math.abs(v1.getZ()-v2.getZ())<margin){
			return true;
		} else {
			return false;
		}
